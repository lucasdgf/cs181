...
class MajorityLearner(Learner):
    ...
    def train(self, dataset):
        ...
        self.most_popular = mode([e.attrs[dataset.target] for e in dataset.examples])
...
class DecisionTree:
    ...
    def score(self, dataset):
        """Given a dataset, returns 0-1 accuracy of the tree on the dataset."""
        total, correct = 0, 0
        for e in dataset.examples:
            total += 1
            if self.predict(e) == e.attrs[-1]:
                correct += 1
        if total == 0:
            return 0
        else:
    ...
    def prune(self, validation):
        if self.nodetype == DecisionTree.NODE:
            prune_status = True
            for branch in self.branches:
                if self.branches[branch].nodetype == DecisionTree.NODE:
                    self.branches[branch].prune(validation)
                    if self.branches[branch].nodetype == DecisionTree.NODE:
                        prune_status = False
            if prune_status:
                most_popular = mode([self.branches[b].classification for b in self.branches])
                majority = DecisionTree(DecisionTree.LEAF, classification=most_popular)
                if majority.score(validation) >= self.score(validation):
                    self.nodetype = DecisionTree.LEAF
                    self.classification = most_popular
...
class DecisionTreeLearner(Learner):
    ...
    def decision_tree_learning(self, examples, attrs, depth, default=None):
        if depth == 0:
            if not default:
                default = self.majority_value(examples)
    ...
    def information_gain(self, attr, examples):
        ...
        # W = sum([e.weight for e in examples])
        return I(examples) - float(remainder) / (sum([e.weight for e in examples]))
